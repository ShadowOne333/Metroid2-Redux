...
RST_10::
    ld a, [bankRegMirror]
    jp switchToBank10GBC

SECTION "RST_18", ROM0[$0018]
RST_18::
    ld a, [bankRegMirrorGBC]
    jp returnFromBank10GBC
...
LCDCInterrupt::
    jp LCDCInterruptHandler_farCall
    
SECTION "TimerOverflowInterrupt", ROM0[$0050]
TimerOverflowInterrupt::
    jp TimerOverflowInterruptStub

SECTION "SerialTransferCompleteInterrupt", ROM0[$0058]
SerialTransferCompleteInterrupt::
    jp SerialTransferInterruptStub

SECTION "Bank10GBC", ROM0[$0068]
switchToBank10GBC:
    ld [bankRegMirrorGBC], a
    .noMirror:
    switchBankVar $10
    ret
...
gfxInfoSmall_enemiesA: db BANK(gfx_enemiesA)
    dw gfx_enemiesA, colorid_enemiesA

gfxInfoSmall_enemiesB: db BANK(gfx_enemiesB)
    dw gfx_enemiesB, colorid_enemiesB

gfxInfoSmall_enemiesC: db BANK(gfx_enemiesC)
    dw gfx_enemiesC, colorid_enemiesC

gfxInfoSmall_enemiesD: db BANK(gfx_enemiesD)
    dw gfx_enemiesD, colorid_enemiesD

gfxInfoSmall_enemiesE: db BANK(gfx_enemiesE)
    dw gfx_enemiesE, colorid_enemiesE

gfxInfoSmall_enemiesF: db BANK(gfx_enemiesF)
    dw gfx_enemiesF, colorid_enemiesF

gfxInfoSmall_arachnus: db BANK(gfx_arachnus)
    dw gfx_arachnus, colorid_arachnus

gfxInfoSmall_surfaceSPR: db BANK(gfx_surfaceSPR)
    dw gfx_surfaceSPR, colorid_surfaceSPR

gfxInfoSmall_plantBubbles:: db BANK(gfx_plantBubbles)
    dw gfx_plantBubbles, $0000

gfxInfoSmall_ruinsInside:: db BANK(gfx_ruinsInside)
    dw gfx_ruinsInside, $0000

gfxInfoSmall_queenBG:: db BANK(gfx_queenBG)
    dw gfx_queenBG, $0000

gfxInfoSmall_caveFirst:: db BANK(gfx_caveFirst)
    dw gfx_caveFirst, $0000

gfxInfoSmall_surfaceBG:: db BANK(gfx_surfaceBG)
    dw gfx_surfaceBG, $0000

gfxInfoSmall_lavaCavesA:: db BANK(gfx_lavaCavesA)
    dw gfx_lavaCavesA, $0000

gfxInfoSmall_lavaCavesB:: db BANK(gfx_lavaCavesB)
    dw gfx_lavaCavesB, $0000

gfxInfoSmall_lavaCavesC:: db BANK(gfx_lavaCavesC)
    dw gfx_lavaCavesC, $0000

gfxInfoSmall_metAlpha:: db BANK(gfx_metAlpha)
    dw gfx_metAlpha, colorid_metAlpha

gfxInfoSmall_metGamma:: db BANK(gfx_metGamma)
    dw gfx_metGamma, colorid_metGamma

gfxInfoSmall_metZeta:: db BANK(gfx_metZeta)
    dw gfx_metZeta, colorid_metZeta

gfxInfoSmall_metOmega:: db BANK(gfx_metOmega)
    dw gfx_metOmega, colorid_metOmega
    
gfxInfoSmall_ruinsExt:: db BANK(gfx_ruinsExt)
    dw gfx_ruinsExt, $0000

gfxInfoSmall_finalLab:: db BANK(gfx_finalLab)
    dw gfx_finalLab, $0000

gfxInfo_queenSPR:: db BANK(gfx_queenSPR)
    dw gfx_queenSPR, vramDest_enemies, $0500, colorid_queenSPR

gfxInfo_commonItems:: db BANK(gfx_commonItems)
    dw gfx_commonItems, vramDest_commonItems, $0100, colorid_commonItems
...
VBlankHandler:
    ...
    ldh [rSCX], a
    ; Update palettes
    ld a, BANK(VBlank_checkTransferColors)
    ld [rMBC_BANK_REG], a
    call VBlank_checkTransferColors 
...
HBlank_copyToVram:
    .loop:
        ...
        ld a, [hl]
        ld [de], a
        ; write twice in case interrupts delayed the first write enough for it to occur outside of hblank
        .waitForHBlank2:
            ldh a, [rSTAT]
            and $03
            jr nz, .waitForHBlank2
        ld a, [hl+]
        ld [de], a
        inc de
        dec bc
...
.end:
    ; Read destination address of transfer (and transform to d200-d37f)
    swap d
    swap e
    ld a, d
    and $f0
...
VBlank_copyGfxInfo: ;{ 00:3C3F
    call copyGfxInfo.start
    ; remember hl for color id transfer
    push bc
    push de
    push hl
    call copyGfxInfo.mid
    .DMAAndCopyColor:
    ; transfer gfx
    call VBlank_DMAToVram
    ; color id transfer
    pop hl
    ld a, [hl+]
    ld b, a
    ...
    pop de
    pop bc
    call copyGfxInfo.end
    jp copyToVram

.small:
    call copyGfxInfo.smallStart
    push bc
    push de
    push hl
    ld h, a
...
; hl: source, de: destination, bc: length
VBlank_DMAToVram: ;{ 00:0393
    call DMAToVram.start
    push bc
    call DMAToVram.mid
    and $7f
    ldh [rHDMA5], a
...
; hl: source, de: destination, bc: length
HBlank_DMAToVram: ;{ 00:0393
    ; for reset bank later on
    ld a, [bankRegMirror]
    ld [vramTransfer_srcBank], a
    call DMAToVram.start
    push bc
    ...
    or $80
    ld b, a
    ; cant start hblank transfer during hblank
    .waitForHBlank:
        ldh a, [rSTAT]
        and $03
        jr nz, .waitForHBlank
    ...
    ldh a, [rLY]
    cp $8f
    jr c, .loop_checkTransferEnd
        push de
        push hl
        ; pause hblank transfer
        ldh a, [rHDMA5]
        ldh [rHDMA5], a
        
        call waitOneFrame
        switchBankVar [vramTransfer_srcBank] ; reset bank since waitOneFrame sets it to 4
        ; unpause transfer, restore transfer progress
        ldh a, [rHDMA5]
        or $80 ; this line is only here to circumvent a glitch in mgba 0.10.2 where bit 7 was incorrectly unset when hblank dma is paused
        ld b, a
        pop hl
        pop de
        jr .waitForHBlank
...
gameMode_LoadA: ;{ 00:03B5
    ...
    jr nz, .tiletableLoop
    ld d, HIGH(tiletableColorArray)
    .tiletableColorsLoop:
        ld a, [hl+]
        ld [de], a
        inc de
        ld a, d
        cp HIGH(tiletableColorArray.end) ;$d8
    jr nz, .tiletableColorsLoop

    ld d, HIGH(collisionArray)
...
; Load various variables from saveBuf to their working copies
    ...
    ld [samus_onTheFlyGfx_indexOld], a ; load first samus sprite
...
mapUpdate_writeToBuffer: ;{ 00:0886
    ...
    ld l, a
    ld a, h
    xor (HIGH(tiletableArray) ^ HIGH(tiletableColorArray))
    ld h, a
    ld a, [hl+]
    ld [tempMetatile.topLeftColor], a
    ld a, [hl+]
    ld [tempMetatile.topRightColor], a
    ld a, [hl+]
    ld [tempMetatile.bottomLeftColor], a
    ld a, [hl]
    ld [tempMetatile.bottomRightColor], a
    ...
    ld de, tempMetatile
    .loop:
        ; load tile id into mapUpdateBuffer
        ld a, [de]
        ld [hl+], a
        inc e

        ; load color id into mapUpdateBuffer
        ld a, [de]
        ld [hl+], a
        inc e

        bit 3, e
    jr z, .loop ; exit when all 4 tiles of the block are processed
...
VBlank_updateMap: ;{
    push bc
    ld bc, $011e
    ld de, mapUpdateBuffer - 1

    .loop:
        ; load VRAM background tilemap destination address into hl
        inc de
        ld a, [de]
        ld l, a
        inc e
        ...
        ld h, a
        and a
        jr z, .loopExit ; exit it address is $00xx (update terminated)

        call .writeOneTileAndColorID ; top left tile
        call .writeOneTileAndColorID ; top right tile

        ld a, l ; move to bottom left tile
        add c
        ld l, a

        call .writeOneTileAndColorID ; bottom left tile
        call .writeOneTileAndColorID ; bottom right tile
    jr .loop
...
.writeOneTileAndColorID:
    inc de
    ld a, [de]
    ld [hl], a ; write tile id
    ld a, b
    ldh [rVBK], a ; change vram bank to 1 (color specification)
    inc e
    ld a, [de]
    ld [hl+], a ; write color id
    xor a
    ldh [rVBK], a ; restore vram bank to 0
ret
;}
...
.toggleMissiles: ; 00:2212
    ...
    .else:
        ...
        ld a, $08
        ld [samusActiveWeapon], a
        ld hl, gfxInfo_cannonMissile_BufferTop
        call HBlank_copyGfxInfo
        ld hl, gfxInfo_cannonMissile_BufferBottom
        call HBlank_copyGfxInfo
        ; Play sound effect
        ld a, $15
        ld [sfxRequest_square1], a
...
; 00:2242
gfxInfo_cannonMissile_BufferTop: db BANK(gfx_cannonMissile)
    dw gfx_cannonMissile, vramDest_cannon, $0020, colorid_cannonMissile
gfxInfo_cannonMissile_BufferBottom: db BANK(gfx_cannonMissile)
    dw gfx_cannonMissile, vramDest_cannon + $0100, $0020, colorid_cannonMissile
; 00:2249
gfxInfo_cannonBeam_BufferTop: db BANK(gfx_cannonBeam) 
    dw gfx_cannonBeam, vramDest_cannon, $0020, colorid_cannonBeam
gfxInfo_cannonBeam_BufferBottom: db BANK(gfx_cannonBeam) 
    dw gfx_cannonBeam, vramDest_cannon + $0100, $0020, colorid_cannonBeam
...
.doorToken_copy:
    ...
    jr nz, .doorToken_damage
        ...
        push hl
            call HBlank_loadDefaultHud_longJump
        pop hl

        ; Set the loadSpawnFlagsRequest flag (why does zero == true for this variable?)
        xor a
        ld [loadSpawnFlagsRequest], a
        ld [queen_headFrameNext], a ; fix bug from vanilla metroid 2: if this isnt zero when you escape, hud will fail to update properly
    jp .nextToken ;}
    ...
    ;jp nz, .nextToken
        ...
        push hl
            ...
            call HBlank_DMAToVram
            pop af
            ; Multiply by 4 for offset to colorid
            rlca
            rlca
            ld e, a
            ld d, $00
            ld hl, colorid_items
            add hl, de
            ld de, colorIDBuffer + ((vramDest_item >> 4) & $01ff)
            ld bc, $0004
            call copyToVram

        ; Load item orb {
            ; no need to switch bank bc itemorb is in same bank as items
            ld hl, gfxInfo_itemOrb
            call HBlank_copyGfxInfo
        ;}
        ...
        ; Load item text {
        ; Set source bank
        switchBank itemTextPointerTable
        ; Read lower nybble of token
        ld a, [hl+]
        push hl
            and $0f
...
door_loadGraphics: ;{ 00:26EB
    ...
    .color:
        ld a, [hl+]
        ld [saveBuf_colorSrcLow], a
        ld a, [hl+]
        ld [saveBuf_colorSrcHigh], a
...
door_loadCollision: ;{ 00:2859
    ...
    push hl
        ...
        jr nz, .loop
        dec de
        ld a, $00 ; collision of air tiles
        ld [de], a
    pop hl
ret ;}
...
VBlank_vramDataTransfer: ;{ 00:2BA3
    ...
    rst $00
    /*ld a, BANK(VBlank_transferData)
    ld [rMBC_BANK_REG], a
    call VBlank_transferData 
...
VBlank_variaAnimation: ;{ 00:2BF4
    ...
    jr nz, .endIf_A
        ...
        ld b, a
        ld hl, .variaAnimationTileNumbersTable
        ; b is current destination, a is potential threshold
        ; b must be < a. while b >= a, loop 
        .loop_findByteThreshold:
            ld a, [hl+]
            cp b
        jr c, .loop_findByteThreshold
        jr z, .loop_findByteThreshold
        ...
        .loop:
            ...
            add $10
            ld l, a
            ; Exit loop if >= threshold
            cp b
        jr c, .loop
        ; if current position - 0x0f == threshold, it means we switch to next set of tiles
        sub $0f
        ld b, a
        ld hl, .variaAnimationTileNumbersTable
        ; b is current destination, a is potential threshold
        .loop_findByteThreshold2:
            ld a, [hl+]
            cp b
        jr c, .loop_findByteThreshold2
        jr z, .exactThreshold
    ...
    pop af
reti
    .variaAnimationTileNumbersTable:
        db $20, $60, $90, $A0, $B0, $FF
 ;}
...
.exit_noHit:
    ...
    scf
    ccf
ret ;}; Samus-sprite downward collision loop
...
gameMode_dead:
    ...
    ld bc, $0009
    call copyBGTilemapToVramWithColorID 
    ; Reset scroll
    xor a
    ld [scrollY], a
    ld [scrollX], a
...
gfxInfo_ice: db BANK(gfx_beamIce)
    dw gfx_beamIce, vramDest_beam, $0020, colorid_beamIce
...
gfxInfo_wave: db BANK(gfx_beamWave)
    dw gfx_beamWave, vramDest_beam, $0020, colorid_beamWave
...
gfxInfo_variaSuit: db BANK(gfx_samusVariaSuit)
    dw gfx_samusVariaSuit, vramDest_samus, $07B0, colorid_samusVariaSuit
;}
...
LCDCInterruptHandler_farCall: ; 00:3D5C
    push af
        ...
        ld a, BANK(LCDCInterruptHandler)
        ld [rMBC_BANK_REG], a
...
samus_onTheFlyGfx_loadGfx_farCall:
    callFar samus_onTheFlyGfx_loadGfx
    switchBank drawSamus_common
ret
...
VBlank_drawCreditsLine_longJump:
    ...
    jp VBlank_updateCreditsLineColor_longJump
...
HBlank_loadDefaultHud_longJump:
    jpLong HBlank_loadDefaultHud
...
copyFromTransferBank:
    switchBankVar [vramTransfer_srcBank]
    call copyToVram
    jp switchToBank10GBC.noMirror ; return to bank 10 function

copyBGTilemapToVramWithColorID:
    push bc
    push de
    call copyToVram
    pop hl
    pop bc
    jpLong updateBGTilemapColorID
...
waitForFrameStart:
    callFar updateColors

    ; wait for the start of drawing the screen, after VBlankHandler is done running
    ; two conditions must be fulfilled:
    ;  - hVBlankDoneFlag must be $01
    ;  - rLY must be < $90

    .loop_A:
        ldh a, [hVBlankDoneFlag]
        or a
        jr nz, .VBlankDone ; exit if VBlankHandler is done running

        ldh a, [rLY]
        cp $8f
        jr nc, .loop_B ; exit if screen is too close to done rendering 
        ; (waiting for interrupt at this point could probably miss the vblank interrupt about to fire)
    ...
    .loop_B:
        ldh a, [hVBlankDoneFlag]
        or a
    jr z, .loop_B ; exit if VBlankHandler is done running
    ; fallthrough

.VBlankDone:
    ; wait for rLY to become < $90
    .loop_C:
        ldh a, [rLY]
        cp $90
    jr nc, .loop_C
...
VBlank_updateCreditsLineColor_longJump:
    ld a, BANK(VBlank_updateCreditsLineColorID)
    ld [rMBC_BANK_REG], a
    call VBlank_updateCreditsLineColorID
