...
drawSamusSprite: ;{ 01:4B62
    ...
    .spriteLoop:
        ...
        ld a, [de]
        ld b, a
        ld a, [drawSamusSpriteTileOffset]
        add b
        pop bc
        ld [hl+], a
...
drawSamus_knockback: ;{ 01:4C59 - $0F, $11: Knockback
    ...
    ld a, [samusFacingDirection]
    ld e, a
    ld hl, .knockbackSpriteIdTable
    add hl, de
    ; Load sprite
    ld a, [hl]
    ldh [hSpriteId], a

    ld hl, .knockback_onTheFlyGfx_indexTable
    add hl, de
    ld a, [hl]
    ld [samus_onTheFlyGfx_indexNew], a
jp drawSamus_common

.knockbackSpriteIdTable: ; 02:4C69
    db $16, $09
.knockback_onTheFlyGfx_indexTable: ; 02:4C69
    db $14, $15
;}
...
drawSamus_jump: ;{ 01:4CBD - $01, $07: Jumping and falling
    ...
    ldh a, [hTemp.a]
    ld e, a
    ld hl, jumpSpriteIdTable
    add hl, de
    ; Get sprite ID
    ld a, [hl]
    ldh [hSpriteId], a

    ld hl, jump_onTheFlyGfx_indexTable
    add hl, de
    ld a, [hl]
    ld [samus_onTheFlyGfx_indexNew], a
jp drawSamus_common ;}
...
jumpSpriteIdTable: ; 01:4CDE
; Value read is based on input and facing direction
;                                U    U              D    D
;       x    R    L    x    U    R    L    x    D    R    L 
    db $00, $09, $16, $00, $00, $0a, $17, $00, $00, $0c, $19
jump_onTheFlyGfx_indexTable:
    db $00, $15, $14, $00, $00, $17, $16, $00, $00, $18, $18
...
drawSamus_standing: ;{ $00: Standing
    ...
    ldh a, [hTemp.a]
    ld e, a
    ld hl, .standingSpriteIdTable
    add hl, de
    ; Load sprite ID
    ld a, [hl]
    ldh [hSpriteId], a

    ld hl, .standing_onTheFlyGfx_indexTable
    add hl, de
    ld a, [hl]
    ld [samus_onTheFlyGfx_indexNew], a
jp drawSamus_common

.standingSpriteIdTable: ; 01:4D54
; Value read is based on input and facing direction
;                                U    U              D    D
;       x    R    L    x    U    R    L    x    D    R    L
    db $00, $01, $0e, $00, $00, $02, $0f, $00, $00, $01, $0e
.standing_onTheFlyGfx_indexTable:
    db $00, $02, $01, $00, $00, $04, $03, $00, $00, $02, $01
;}
...
drawSamus_run: ;{ 01:4D77 - $03: Running
    ...
    ld hl, .runningSpriteIdTableNormal
    ld bc, .running_onTheFlyGfx_indexTableNormal
    ldh a, [hInputPressed]
    bit PADB_UP, a
    jr z, .else_C
        ld hl, .runningSpriteIdTableAimingUp
        ld bc, .running_onTheFlyGfx_indexTableAimingUp
        jr .endIf_C
    .else_C:
        ldh a, [hInputPressed]
        bit PADB_B, a
        jr z, .endIf_C
            ld hl, .runningSpriteIdTableShooting
            ld bc, .running_onTheFlyGfx_indexTableShooting
    .endIf_C:
    add hl, de
...
.runningSpriteIdTableNormal: ; 01:4DC7 - Normal
    db $10, $11, $12, $00 ; Left
    db $03, $04, $05, $00 ; Right
.runningSpriteIdTableShooting: ; 01:4DCF - Firing forwards
    db $13, $14, $15, $00 ; Left
    db $06, $07, $08, $00 ; Right
.runningSpriteIdTableAimingUp: ; 01:4DD7 - Aiming up
    db $2e, $2f, $30, $00 ; Left
    db $2b, $2c, $2d, $00 ; Right

.running_onTheFlyGfx_indexTableNormal: ; 01:4DC7 - Normal
    db $05, $0A, $0F, $00 ; Left
    db $05, $0A, $0F, $00 ; Right
.running_onTheFlyGfx_indexTableShooting: ; 01:4DCF - Firing forwards
    db $06, $0B, $10, $00 ; Left
    db $07, $0C, $11, $00 ; Right
.running_onTheFlyGfx_indexTableAimingUp: ; 01:4DD7 - Aiming up
    db $08, $0D, $12, $00 ; Left
    db $09, $0E, $13, $00 ; Right
;}
...
drawSamus_common: ;{ 01:4DDF
    ...
    ld b, a
    ld a, [samus_onTheFlyGfx_indexOld]
    cp b
    jr z, .noChangeInSamusGfxInfo
        ld a, [samusVRAMBuffer]
        xor $01
        ld [samusVRAMBuffer], a
        
        ld a, b
        ld [samus_onTheFlyGfx_indexOld], a

        ; load gfx
    	call samus_onTheFlyGfx_loadGfx_farCall
    .noChangeInSamusGfxInfo:

    ; load sprite offset
    ld a, [samusVRAMBuffer]
    swap a
    ld [drawSamusSpriteTileOffset], a
    ...
    ldh [hSpriteAttr], a
    ld [drawSamusSpriteTileOffset], a
    inc a ; this used to be done in drawHudMetroid, which i removed
